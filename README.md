...
A ChatGPT account with Codex access is required. Use **Sign in** in the popup and complete OpenAI's browser authorization flow. The extension uses OAuth authorization code flow with PKCE and captures Codex's localhost callback from the sign-in tab. The Codex refresh token is stored in extension-local browser storage that is restricted to trusted extension pages; short-lived access tokens stay in restricted in-memory session storage. Only the background service refreshes or uses these tokens. **Sign out** stops active requests, asks OpenAI to revoke the refresh token, and always removes both tokens locally. If OpenAI cannot confirm revocation, the popup says that this was only a local disconnect and points to ChatGPT security settings. The agent currently uses `gpt-5.6-sol` through the user's Codex subscription.
